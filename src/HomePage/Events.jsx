...
export default function Events() {
  ...
  // ScrollTrigger animation for header text and subtitle
  useEffect(() => {
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-8 sm:py-12 px-4">
      <div className="max-w-7xl mx-auto text-center mb-8 sm:mb-12">
        <h2
          ref={titleRef}
          className="text-3xl sm:text-4xl lg:text-5xl font-bold text-black mb-4 opacity-0"
        >
          Upcoming Events
        </h2>
        <p
          ref={subtitleRef}
          className="text-gray-600 text-sm sm:text-base lg:text-lg max-w-2xl mx-auto opacity-0"
        >
      ...
      <div className="max-w-7xl mx-auto relative">
        <div className="flex justify-between items-center mb-6 sm:mb-8">
          <button
            onClick={prevSlide}
            className="group flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-white rounded-full shadow-lg border border-gray-200 hover:bg-gray-50"
          ...
          <div className="flex items-center space-x-4">
            <button
              ...
              className="group flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 bg-white rounded-full shadow-md border border-gray-200 hover:bg-gray-50"
              aria-label={isPlaying ? 'Pause auto slide' : 'Play auto slide'}
            >
              {isPlaying ? (
                <Pause className="w-4 h-4 sm:w-5 sm:h-5 text-gray-600" />
              ) : (
                <Play className="w-4 h-4 sm:w-5 sm:h-5 text-gray-600 ml-0.5" />
              )}
            </button>
        ...
        <div className="overflow-hidden">
          ...
          >
            {Array.from({ length: totalSlides }).map((_, slideIndex) => (
              <div
                key={slideIndex}
                className="w-full flex-shrink-0 flex gap-4 sm:gap-6 lg:gap-8 px-2 justify-center"
              >
                {events
                  ...
                  .map((event, cardIndex) => (
                    <div
                      key={cardIndex}
                      className="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden transform hover:-translate-y-2 border border-gray-100 w-full sm:w-[calc(50%-0.75rem)] lg:w-[calc(33.333%-1rem)]"
                    >
                      <div className="relative overflow-hidden">
                        ...
                        <div className="absolute top-4 left-4">
                          <span
                            className={`px-3 py-1 rounded-full text-xs font-semibold ${categoryColors[event.category]}`}
                          >
                            {event.category}
                          </span>
                        </div>
